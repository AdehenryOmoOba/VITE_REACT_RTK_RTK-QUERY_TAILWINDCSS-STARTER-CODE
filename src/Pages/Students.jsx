...
import { openModal,closeModal } from '../rtk/features/modal/modalSlice'
...
function Students() {
  ...
  if (isFetching) return <div className='w-full'><h2 className='text-black text-xl text-center'>Loading...</h2></div>
    ...
    return (
            <div className='w-full grid relative grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 p-8 gap-4 ml-10 md:ml-4 lg:ml-0'>
              {data?.map((student) => (<Card key={student.id} data={student}/>))}
              <div className='grid absolute h-10 w-10 rounded-full bg-blue-600 place-content-center bottom-4 right-4 cursor-pointer' onClick={() => dispatch(openModal())}>
